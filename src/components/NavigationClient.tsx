...
export default function NavigationClient({ logo, headerData, services = [] }: NavigationClientProps) {
  ...
  // Use default navigation if headerData is not provided
  ...
  const searchConfig = headerData?.search || {
    enabled: true,
    placeholder: "Search",
  };
      ...
      <div className={themeStyles.layout.container}>
        <div className="flex justify-between items-center py-4 sm:py-5">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {/* Services dropdown (if services available) */}
            {services.length > 0 && (
              <div
                className="relative"
                ...
                {servicesOpen && (
                  <div className="absolute left-0 mt-2 w-72 bg-white border border-neutral-200 shadow-lg rounded-xl p-3 grid grid-cols-1 gap-1 z-40">
                    {services.map(service => (
                      <Link key={service.id} href={`/services/${service.id}`} className="px-3 py-2 rounded-md text-sm text-neutral-700 hover:bg-neutral-100 hover:text-primary-600 transition-colors">
                        {service.title}
                      </Link>
                    ))}
            ...
            {navigation.map((link) => (
              <Link 
                key={link.href} 
                href={link.href}
                className={themeStyles.navigation.link}
            ...
            {searchConfig.enabled && (
              ...
              >
                ...
                <span>{searchConfig.placeholder}</span>
              </button>
            )}
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-neutral-200 py-4">
            <nav className="flex flex-col space-y-4">
              {/* Services dropdown for mobile (if services available) */}
              {services.length > 0 && (
                <details className="px-2">
                  ...
                  <div className="mt-2 ml-2 flex flex-col gap-1">
                    {services.map(service => (
                      <Link key={service.id} href={`/services/${service.id}`} className="text-sm px-3 py-2 rounded-md hover:bg-neutral-100"
                        onClick={() => setMobileMenuOpen(false)}>{service.title}</Link>
                    ))}
              ...
              {/* Dynamic navigation links for mobile */}
              {navigation.map((link) => (
                <Link 
                  key={link.href} 
                  href={link.href} 
                  className={`${themeStyles.navigation.link} px-4 py-2`} 
              ...
              {ctaButton.enabled && (
                <Link
                  href={ctaButton.href}
                  className="mx-4 mt-2 inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 transition-colors"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {ctaButton.label}
                </Link>
